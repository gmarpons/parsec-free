...
runPT :: (Monad m, Stream s m t, Show t)
      => ParsecT s u m a -> u -> SourceName -> s -> m (Either ParseError a)
runPT = P.runPT . F.eval

runPTLog :: (MonadIO m, MonadReader F.LogType m, Stream s m t, Show t)
         => ParsecT s u m a -> u -> SourceName -> s
         -> m (Either ParseError a)
runPTLog = P.runPT . F.evalLog

runP :: (Stream s Identity t, Show t)
     => Parsec s u a -> u -> SourceName -> s -> Either ParseError a
runP p u n s = runIdentity $ P.runPT (F.eval p) u n s

runParserT :: (Stream s m t, Show t)
           => ParsecT s u m a -> u -> SourceName -> s -> m (Either ParseError a)
runParserT = runPT

runParserTLog :: (MonadIO m, MonadReader F.LogType m, Stream s m t, Show t)
              => ParsecT s u m a
              -> u
              -> SourceName
              -> s
...
runParser :: (Stream s Identity t, Show t)
          => Parsec s u a -> u -> SourceName -> s -> Either ParseError a
runParser = runP

parse :: (Stream s Identity t, Show t)
      => Parsec s () a -> SourceName -> s -> Either ParseError a
parse p = runP p ()

parseTest :: (Stream s Identity t, Show a, Show t)
          => Parsec s () a -> s -> IO ()
parseTest p input
...
parseTestLog' :: (MonadIO m, MonadReader F.LogType m, Stream s m t, Show a, Show t)
              => ParsecT s () m a -> s -> m ()
...
parseTestLog :: (Stream s (ReaderT F.LogType IO) t, Show a, Show t)
